...
public class TagCloudView extends ViewGroup implements Runnable, TagsAdapter.OnDataSetChangeListener {
    private static final float TOUCH_SCALE_FACTOR = .15f;
    private static final float TRACKBALL_SCALE_FACTOR = 3.5f;
...
//            mAngleX = (y) * speed * TOUCH_SCALE_FACTOR;
//            mAngleY = (-x) * speed * TOUCH_SCALE_FACTOR;
    ...
    private void handleTouchEvent(MotionEvent e) {
        ...
        switch (e.getAction()) {
            ...
            case MotionEvent.ACTION_MOVE:
                //rotate elements depending
                //
                // on how far the selection point is from center of cloud
                dx = e.getX() - downX;
                dy = e.getY() - downY;
                if (isValidMove(dx, dy)) {
//                    mAngleX = (dy / radius) * speed * TOUCH_SCALE_FACTOR;
//                    mAngleY = (-dx / radius) * speed * TOUCH_SCALE_FACTOR;
                    mAngleX = (dy / radius) * speed * TRACKBALL_SCALE_FACTOR;
                    mAngleY = (-dx / radius) * speed * TRACKBALL_SCALE_FACTOR;
                    processTouch();
                }
                break;
    ...
    private void hideSpeed(final float xv, final float yv) {
//        new Handler().postDelayed(new Runnable() {
//            @Override
//            public void run() {
//                mAngleX = (yv / radius) * speed * TOUCH_SCALE_FACTOR;
//                mAngleY = (-xv / radius) * speed * TOUCH_SCALE_FACTOR;
//                processTouch();
//            }
//        }, 500);
        s = TRACKBALL_SCALE_FACTOR;

        timer = new CountDownTimer(1000, 10) {
            @Override
            public void onTick(long millisUntilFinished) {
                s = s - 0.15f;
                if (s < TOUCH_SCALE_FACTOR) {
                    s = TOUCH_SCALE_FACTOR;
                }
                mAngleX = (yv / radius) * speed * s;
                mAngleY = (-xv / radius) * speed * s;
                processTouch();
            }
