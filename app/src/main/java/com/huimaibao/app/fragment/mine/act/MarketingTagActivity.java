 ...
 * 标签管理
 */
public class MarketingTagActivity extends BaseActivity implements OnItemClickListener {
    /**
     * 用户标签的GRIDVIEW
     */
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.act_mine_marketing_tag);
     ...
     * 获取营销标签
     */
    private void getMarketTag() {
        GeneralLogic.Instance(mActivity).getProfleLabelApi(new ResultBack() {
            @Override
                ...
                try {
                    ...
                    if (status.equals("0")) {
                        ...
                        for (int i = 0; i < arraySet.length(); i++) {
                            ...
                            item.setId(i + 1);
                            item.setName(arraySet.getJSONObject(i).getString("name"));
                            item.setOrderId(i + 1);
                            item.setSelected(1);
                        ...
                        for (int i = 0; i < arrayUnset.length(); i++) {
                            ...
                            item.setName(arrayUnset.getJSONObject(i).getString("name"));
                            item.setOrderId(i + 1);
                            item.setSelected(0);
                        ...
                        mActivity.runOnUiThread(new Runnable() {
                            @Override
                            public void run() {
                                userAdapter = new DragAdapter(mActivity, userChannelList, false);
    ...
    private void getMarketTagSave() {
        ...
        map.put("label", userChannerIdList.toString().replace("[", "").trim().replace("]", "").trim());
        GeneralLogic.Instance(mActivity).getProfleLabelAddApi(map, new ResultBack() {
            @Override
            public void onSuccess(Object object) {
                try {
                    JSONObject json = new JSONObject(object.toString());
